...
@GodotClass(name = "CSGDemo", parent = "Node")
public class CSGDemo extends Node {

	private static final double ROT_SPEED = 0.003;
	private static final double ZOOM_SPEED = 0.125;
	private static final int MAIN_BUTTONS = 1 | 2 | 4; // LEFT | RIGHT | MIDDLE
	...
	public boolean _unhandledInput(Object inputEvent) {
		...
		if ("InputEventMouseButton".equals(className)) {
			...
			} else if (buttonIndex == 5) { // WHEEL_DOWN
				cameraDistance += ZOOM_SPEED;
			}
	...
	public void _process(double delta) {
		...
		double currentZ = holderPos.getZ();
		double targetZ = testerPos.getZ();
		double newZ = lerp(currentZ, targetZ, 3 * delta);
