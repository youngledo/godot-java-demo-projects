...
public class BeachCave extends Node2D {
	...
	public void _ready() {
		...
		initialized = true;
		cave = (org.godot.Godot) call("get_node", "Cave");
	}
		...
		if ("InputEventMouseMotion".equals(className)) {
			...
			if (buttonMask > 0 && cave != null) {
				...
				if (pos != null && relative != null) {
					double newX = Math.max(-CAVE_LIMIT, Math.min(0, pos.getX() + relative.getX()));
					cave.setProperty("position", new Vector2(newX, pos.getY()));
				}
			}
		}

		if ((boolean) ev.call("is_action_pressed", "toggle_glow_map")) {
